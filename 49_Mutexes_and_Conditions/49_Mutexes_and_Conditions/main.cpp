...
int threadCallback(void* pData);

// Our worker thread
int worker(void* pData);
...
SDL_SpinLock gDataSpinLock = 0;

// Our worker functions
int producer(void *pData);
int consumer(void *pData);
void produce();
void consume();
...
int main(int argc, char* args[])
{
	...
	else
	{
		...
		if (LoadMedia())
		{
			...
			// Run the thread
			//int data = 101;
			//SDL_Thread* threadID = SDL_CreateThread(threadCallback, "LazyThread", (void*)data);
...
bool LoadMedia()
{
	...
	gSplashTexture.loadFromFile("splash.png");
...
int worker(void* pData)
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		// Unlock
		SDL_AtomicUnlock(&gDataSpinLock);
...
void consume()
{
	...
	if (gData == -1)
	{
		...
		printf("\nProducer encountered empty buffer, waiting for producer to fill buffer...\n");
...
{
	// Show the dot
	gDotTexture.render((int)m_PosX, (int)m_PosY);
