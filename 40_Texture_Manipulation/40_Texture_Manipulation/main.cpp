...
bool LoadMedia()
{
	...
	else
	{
		...
		{
			SDL_Log("Unable to lock Foo' texture!\n");
		}
		else // Manual color key
		{
			// Allocate format from window
			Uint32 format = SDL_GetWindowPixelFormat(gWindow.getWindow());
...
bool LTexture::loadFromFile(const char* pPath)
{
	...
	else
	{
		...
		SDL_Surface* formattedSurface = SDL_ConvertSurfaceFormat(loadedSurface, SDL_GetWindowPixelFormat(gWindow.getWindow()), 0);
		if (!formattedSurface)
		{
