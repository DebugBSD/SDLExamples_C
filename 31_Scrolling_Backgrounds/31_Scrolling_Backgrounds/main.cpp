	...
	else
	{
		...
		if (LoadMedia())
		{
			...
			while (!quit)
			{
				...
				--scrollingOffset;
				if (scrollingOffset < -gBKGTexture.getWidth())
				{
					scrollingOffset = 0;
				}
				...
				// Render background
				gBKGTexture.render(scrollingOffset, 0);
				gBKGTexture.render(scrollingOffset + gBKGTexture.getWidth(), 0);
...
bool LoadMedia(void)
{
	...
	if (!gBKGTexture.loadFromFile("bg.png"))
	{
		success = false;
	}
